...
int backtracking(Ponto locais[nPontos],int coliders[nColiders][2],
    int disp[nDispositivos],int nPontos,int nColiders,int nDispositivos,int iterador,int counter, int visit[nPontos]){
    ...
    for(i = 0; i<nPontos; i++) {
        if(visit[i] == 0) { //caso nao tenha sido visitado
            ...
            counter = intersectGlobal(coliders, disp);
...
int intersectGlobal(int coliders[nColiders][2], int disp[nDispositivos], Ponto locais[nPontos]) {
    int i, j;
    int acum = 0;
